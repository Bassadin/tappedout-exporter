...
Deno.test("runBackup writes card and metadata changes without rewriting unchanged decks", async () => {
  ...
  try {
    await withMockedFetch(async (setDescription) => {
      assertEquals(await runBackup(config(outputDir)), { discovered: 1, changed: 1, unchanged: 0 });
      assertEquals(await runBackup(config(outputDir)), { discovered: 1, changed: 0, unchanged: 1 });
